<script>
	...
	onMount(() => {
		const onScroll = () => {
			...
			const trigger = stories.getBoundingClientRect().top;

			// switch before stories fully arrive
			mode = trigger < window.innerHeight * 0.65
				? "injunctions"
				: "area";
	...
	const injunctionScale = d3.scaleThreshold()
		.domain([1, 2, 4, 7, 10, 15, 20])
		.range([
			"#0a061b",
			"#2a0a3a",
			"#61187a",
	...
	onMount(async () => {
		...
		canvas.addEventListener("mousemove", (e) => {
			...
			const nextLad = getFeatureAtPoint(
				x,
				y,
				hitContext,
				colorToLad
			...
			const nextWard = getFeatureAtPoint(
				x,
				y,
				wardHitContext,
				colorToWard
	...
	const injunctionCount = $derived(
		hoveredWard?.properties?.injunction_names ?? null
	);
...
<div class="map-wrap">
	...
	<canvas bind:this={canvas} class="base"></canvas>

	<canvas bind:this={hoverCanvas} class="hover"></canvas>

	<canvas bind:this={hitCanvas} style="display:none;"></canvas>

	<canvas bind:this={wardHitCanvas} style="display:none;"></canvas>
